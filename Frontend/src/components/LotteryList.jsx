      ...
      <div className="w-full mb-6 p-2">
        ...
        <div className="bg-black p-3 px-5 w-max rounded-3xl mt-5 shadow-2xl">
          <button
                    ...
                    onClick={()=> {
                      if(ticketStatus === 0)
                        updateTicketStatus(lotteryID,'open')
                      if(ticketStatus === 1)
                        updateTicketStatus(lotteryID,'end')
                      if(ticketStatus === 2)
                        updateTicketStatus(lotteryID,'draw')
                    }}>
                        {ticketStatus === 0 && 'Open Lottery'}
                        {ticketStatus === 1 && 'End Lottery'}
                        {ticketStatus === 2 && 'Draw Lottery'}
                    </button>
        </div>
...
const LotteryList = () => {
  const { currentAccount, lotteryList, endedLotteryList, isOwnerAccount, updateTicketStatus } = useContext(LotteryContext);
  return (
    <div className="flex w-full jusitfy-center items-center 2xl:px-20 gradient-bg-transactions">
      <div className="flex flex-col md:p-12 py-12 px-4">
        ...
        {currentAccount ? (
          ...
          <h3 className="text-white text-3xl text-center my-2">
            Connect Your Account to see the Lotteries
          </h3>
        )}
        <div className="flex flex-wrap justify-center items-center mt-10">
          {lotteryList.reverse().map((trans, index) => (
            <TransactionCard key={index} {...trans} isOwnerAccount={isOwnerAccount} updateTicketStatus={updateTicketStatus} />
          ))}
        </div> 
        {currentAccount ? (
          <h3 className="text-white text-3xl text-center my-2">
        ...
        <div className="flex flex-wrap justify-center items-center mt-10">
          {endedLotteryList.reverse().map((trans, index) => (
            <TransactionCard key={index} {...trans} isOwnerAccount={isOwnerAccount} updateTicketStatus={updateTicketStatus} />
          ))}
        </div> 
      </div>
